...
def generate_placeholder_page(title, filename):
    ...
    breadcrumbs_full = f'<a href="#" class="hover:text-primary transition-colors whitespace-nowrap">Hệ thống & Hỗ trợ</a> <span class="material-symbols-outlined text-sm">chevron_right</span> <span class="text-[#0d121c] whitespace-nowrap">{title}</span>'
    ...
    # We also need to activate the right sidebar link!
    # Let's find the specific <li> link and add the active class
    
    final_content = content[:start_idx] + new_section + content[end_idx:]
    
    # Active state swap
    # By default the global script uses 'inactive_class' for all of them.
    ...
    # Replace the inactive class with active class for this specific href
    target_link_str = f'href="{filename}" class="{inactive_class}"'
    replacement_link_str = f'href="{filename}" class="{active_class}"'
    final_content = final_content.replace(target_link_str, replacement_link_str)
...
def main():
    ...
    for title, filename in pages:
        generate_placeholder_page(title, filename)
